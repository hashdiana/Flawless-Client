...
import me.flawless.api.interfaces.IShaderEffect;
...
public class ShaderManager implements Wrapper {
    ...
    public void applyFlow(Runnable runnable) {
        ...
        if (effect != null)
            ((IShaderEffect) effect).flawless$addFakeTargetHook("bufIn", shaderBuffer);
    ...
    public void setupShader(Shader shader, ManagedShaderEffect effect) {
        me.flawless.mod.modules.impl.render.Shader shaderChams = me.flawless.mod.modules.impl.render.Shader.INSTANCE;
    ...
    public void reloadShaders() {
        ...
        DEFAULT_OUTLINE = ShaderEffectManager.getInstance().manage(new Identifier("shaders/post/outline.json"), managedShaderEffect -> {
            ...
            ((IShaderEffect) effect).flawless$addFakeTargetHook("bufIn", mc.worldRenderer.getEntityOutlinesFramebuffer());
            ((IShaderEffect) effect).flawless$addFakeTargetHook("bufOut", mc.worldRenderer.getEntityOutlinesFramebuffer());
        });

...
import me.flawless.mod.commands.impl.*;
...
public class CommandManager implements Wrapper {
	...
	public final PingCommand ping = new PingCommand();
	...
	public final TCommand t = new TCommand();
	...
	public CommandManager() {
		...
		{
			System.out.println("Error initializing " + FlawLess.LOG_NAME + " commands.");
	...
	public void command(String[] commandIn) {
		...
		Command command = commands.get(commandIn[0].substring(FlawLess.PREFIX.length()).toLowerCase());
	...
	public static void sendChatMessage(String message) {
		...
		if (ChatSetting.INSTANCE.messageStyle.getValue() == ChatSetting.Style.MoonGod) {
			mc.inGameHud.getChatHud().addMessage(Text.of("§f[§b" + ChatSetting.INSTANCE.hackName.getValue() + "§f] " + message));
			return;
		}
		mc.inGameHud.getChatHud().addMessage(Text.of(syncCode + "§r" + ChatSetting.INSTANCE.hackName.getValue() + "§f " + message));
	}

	public static void sendChatMessageWidthId(String message, int id) {
		if (Module.nullCheck()) return;
		if (ChatSetting.INSTANCE.messageStyle.getValue() == ChatSetting.Style.MoonGod) {
			((IChatHud) mc.inGameHud.getChatHud()).flawless$add(Text.of("§f[§b" + ChatSetting.INSTANCE.hackName.getValue() + "§f] " + message), id);
			return;
		}
		((IChatHud) mc.inGameHud.getChatHud()).flawless$add(Text.of(syncCode + "§r" + ChatSetting.INSTANCE.hackName.getValue() + "§f " + message), id);
	}

	public static void sendChatMessageWidthIdNoSync(String message, int id) {
		if (Module.nullCheck()) return;
		((IChatHud) mc.inGameHud.getChatHud()).flawless$add(Text.of("§f" + message), id);
	}
}

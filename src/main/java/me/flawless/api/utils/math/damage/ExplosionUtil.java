package me.flawless.api.utils.math.damage;
...
public class ExplosionUtil implements Wrapper {
    ...
    public static float calculateDamage(double posX, double posY, double posZ, Entity entity, Entity predict, float power) {
        ...
        try {
            blockDensity = getExposure(vec3d, predict);
        } catch (Exception exception) {

        }
        double v = (1.0 - distancedsize) * blockDensity;
        float damage = (int) ((v * v + v) / 2.0 * 7.0 * (double) doubleExplosionSize + 1.0);
        double finald = getBlastReduction((LivingEntity) entity, getDamageMultiplied(damage));
        return (float) finald;
    }

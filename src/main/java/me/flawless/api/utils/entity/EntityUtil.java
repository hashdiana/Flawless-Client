...
public class EntityUtil implements Wrapper {
    ...
    public static void faceVector(Vec3d directionVec) {
        ...
        if (angle[0] == FlawLess.ROTATE.lastYaw && angle[1] == FlawLess.ROTATE.lastPitch) return;
    ...
    public static void sendLook(PlayerMoveC2SPacket packet) {
        if (!packet.changesLook() || packet.getYaw(114514) == FlawLess.ROTATE.lastYaw && packet.getPitch(114514) == FlawLess.ROTATE.lastPitch) {
            return;
        }
        rotating = true;
        FlawLess.ROTATE.setRotation(packet.getYaw(0), packet.getPitch(0), true);

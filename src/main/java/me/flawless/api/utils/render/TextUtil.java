...
public class TextUtil implements Wrapper {
	public static String extractText(OrderedText orderedText) {
	...
	public static boolean isCustomFont() {
		return ClickGui.INSTANCE.font.getValue() && FontRenderers.Arial != null;
	}
	...
	public static void drawString(DrawContext drawContext, String text, double x, double y, int color, boolean customFont) {
		if (customFont) {
			...
			FontRenderers.Arial.drawString(drawContext.getMatrices(), text, (float) x, (float) y + Fonts.INSTANCE.yOffset.getValueFloat() + 2, color);
		} else {
			drawContext.drawText(mc.textRenderer, text, (int) x, (int) y, color, true);
		}
	}

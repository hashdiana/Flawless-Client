...
public class ColorComponents extends Component {
    ...
    public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
        ...
        if (colorSetting.injectBoolean) {
            currentWidth = animate(currentWidth, colorSetting.booleanValue ? (width - 2D) : 0D, ClickGui.INSTANCE.booleanSpeed.getValue());
            switch (ClickGui.INSTANCE.uiType.getValue()) {
                ...
                case New -> TextUtil.drawString(drawContext, colorSetting.getName(), x + 4, y + getTextOffsetY(), colorSetting.booleanValue ? ClickGui.INSTANCE.enableTextS.getValue() : ClickGui.INSTANCE.disableText.getValue());
            }
        } else {
            TextUtil.drawString(drawContext, colorSetting.getName(), x + 4, y + getTextOffsetY(), new Color(-1).getRGB());
        }

...
public class HoleKick extends Module {
    ...
    private final BooleanSetting redStonePacket = add(new BooleanSetting("RedStonePacket", true));
    ...
    private final SliderSetting updateDelay = add(new SliderSetting("UpdateDelay", 100, 0, 500));
    ...
    private final BooleanSetting checkPiston = add(new BooleanSetting("CheckPiston", false));
    private final BooleanSetting autoDisable = add(new BooleanSetting("AutoDisable", true));
    private final BooleanSetting pullBack = add(new BooleanSetting("PullBack", true).setParent());
    private final BooleanSetting onlyBurrow = add(new BooleanSetting("OnlyBurrow", true, v -> pullBack.isOpen()));
    ...
    public HoleKick() {
        super("HoleKick", "use piston push hole fag", Category.Combat);
        INSTANCE = this;
    }
    ...
    public boolean doPush(BlockPos targetPos, PlayerEntity target) {
        ...
        if (!mc.world.getBlockState(targetPos.up(2)).blocksMovement()) {
            for (Direction i : Direction.values()) {
                ...
                if (getBlock(pos) instanceof PistonBlock && (!getBlockState(pos.offset(i, -2)).blocksMovement()) && (BlockUtil.isAir(pos.offset(i, -2).up()) || getBlock(pos.offset(i, -2).up()) == Blocks.REDSTONE_BLOCK)) {
            ...
            if ((BlockUtil.isAir(targetPos) && onlyBurrow.getValue()) || !pullBack.getValue()) {
                if (autoDisable.getValue()) disable();
                return true;
            }
            for (Direction i : Direction.values()) {
            ...
            for (Direction i : Direction.values()) {
                ...
                BlockPos pos = targetPos.offset(i).up();
                for (Direction i2 : Direction.values()) {
                    if (getBlock(pos) instanceof PistonBlock && BlockUtil.isAir(pos.offset(i2))) {
                        if (getBlockState(pos).get(FacingBlock.FACING).getOpposite() != i) continue;

...
public class NameTags extends Module {
    ...
    public void onRender2D(DrawContext context, float tickDelta) {
        for (PlayerEntity ent : mc.world.getPlayers()) {
            if (ent == mc.player && mc.options.getPerspective().isFirstPerson() && FreeCamera.INSTANCE.isOff()) continue;
            ...
            if (vector.z > 0 && vector.z < 1) {
                ...
                if (pops.getValue() && FlawLess.POP.getPop(ent.getName().getString()) != 0) {
                    final_string += " §bPop" + " " + Formatting.LIGHT_PURPLE + FlawLess.POP.getPop(ent.getName().getString());
                }
                ...
                if (font.getValue() == Font.Fancy) {
                    FontRenderers.Arial.drawString(context.getMatrices(), final_string, tagX, (float) posY - 10, FlawLess.FRIEND.isFriend(ent) ? friendColor.getValue().getRGB() : this.color.getValue().getRGB());
                } else {
                    context.getMatrices().push();
                    context.getMatrices().translate(tagX, ((float) posY - 11), 0);
                    context.drawText(mc.textRenderer, final_string, 0, 0, FlawLess.FRIEND.isFriend(ent) ? friendColor.getValue().getRGB() : this.color.getValue().getRGB(), true);
                    context.getMatrices().pop();
                }
                context.getMatrices().pop();
            }

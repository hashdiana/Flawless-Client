...
public class Shader extends Module {
    ...
    public boolean shouldRender(Entity entity) {
        ...
        if (entity instanceof PlayerEntity) {
            ...
            if (FlawLess.FRIEND.isFriend((PlayerEntity) entity))
    ...
    public void onRender3D(MatrixStack matrixStack, float partialTicks) {
        if (hands.getValue())
            FlawLess.SHADER.renderShader(()-> mc.gameRenderer.renderHand(matrixStack, mc.gameRenderer.getCamera(), mc.getTickDelta()), mode.getValue());
    }

    @Override
    public void onToggle() {
        FlawLess.SHADER.reloadShaders();
    }

...
import static me.flawless.api.utils.world.BlockUtil.hasCrystal;
...
public class AutoCrystal extends Module {
    ...
    private final Timer placeTimer = new Timer(), noPosTimer = new Timer(), switchTimer = new Timer(), delayTimer = new Timer();

    private final EnumSetting<Page> page = add(new EnumSetting<>("Page", Page.General));
    //General
    private final BooleanSetting preferAnchor = add(new BooleanSetting("PreferAnchor", true, v -> page.getValue() == Page.General));
    private final BooleanSetting breakOnlyHasCrystal = add(new BooleanSetting("OnlyHold", true, v -> page.getValue() == Page.General));
    private final EnumSetting<SwingSide> swingMode = add(new EnumSetting<>("Swing", SwingSide.Server, v -> page.getValue() == Page.General));
    private final BooleanSetting eatingPause = add(new BooleanSetting("EatingPause", true, v -> page.getValue() == Page.General));
    private final SliderSetting switchCooldown = add(new SliderSetting("SwitchPause", 100, 0, 1000, v -> page.getValue() == Page.General).setSuffix("ms"));
    private final SliderSetting targetRange = add(new SliderSetting("TargetRange", 12.0, 0.0, 20.0, v -> page.getValue() == Page.General).setSuffix("m"));
    private final SliderSetting updateDelay = add(new SliderSetting("UpdateDelay", 50, 0, 1000, v -> page.getValue() == Page.General).setSuffix("ms"));
    private final SliderSetting wallRange = add(new SliderSetting("WallRange", 6.0, 0.0, 6.0, v -> page.getValue() == Page.General).setSuffix("m"));
    private final BooleanSetting debug = add(new BooleanSetting("Debug", false, v -> page.getValue() == Page.General));
    //Rotate
    private final BooleanSetting rotate = add(new BooleanSetting("Rotate", true, v -> page.getValue() == Page.Rotate).setParent());
    private final BooleanSetting onBreak = add(new BooleanSetting("OnBreak", false, v -> rotate.isOpen() && page.getValue() == Page.Rotate));
    private final BooleanSetting yawStep = add(new BooleanSetting("YawStep", false, v -> rotate.isOpen() && page.getValue() == Page.Rotate));
    private final SliderSetting steps = add(new SliderSetting("Steps", 0.3f, 0.1f, 1.0f, 0.01f, v -> rotate.isOpen() && yawStep.getValue() && page.getValue() == Page.Rotate));
    private final BooleanSetting random = add(new BooleanSetting("Random", true, v -> rotate.isOpen() && yawStep.getValue() && page.getValue() == Page.Rotate));
    private final BooleanSetting packet = add(new BooleanSetting("Packet", false, v -> rotate.isOpen() && yawStep.getValue() && page.getValue() == Page.Rotate));
    private final BooleanSetting checkFov = add(new BooleanSetting("CheckLook", true, v -> rotate.isOpen() && yawStep.getValue() && page.getValue() == Page.Rotate));
    private final SliderSetting fov = add(new SliderSetting("Fov", 30f, 0f, 360f, v -> rotate.isOpen() && yawStep.getValue() && checkFov.getValue() && page.getValue() == Page.Rotate));
    //Place
    private final SliderSetting autoMinDamage = add(new SliderSetting("AMin", 5.0, 0.0, 36.0, v -> page.getValue() == Page.Interact).setSuffix("dmg"));
    private final SliderSetting minDamage = add(new SliderSetting("Min", 5.0, 0.0, 36.0, v -> page.getValue() == Page.Interact).setSuffix("dmg"));
    private final SliderSetting maxSelf = add(new SliderSetting("Self", 12.0, 0.0, 36.0, v -> page.getValue() == Page.Interact).setSuffix("dmg"));
    private final SliderSetting range = add(new SliderSetting("Range", 5.0, 0.0, 6, v -> page.getValue() == Page.Interact).setSuffix("m"));
    private final SliderSetting noSuicide = add(new SliderSetting("NoSuicide", 3.0, 0.0, 10.0, v -> page.getValue() == Page.Interact).setSuffix("hp"));
    private final BooleanSetting smart = add(new BooleanSetting("Smart", true, v -> page.getValue() == Page.Interact));
    private final BooleanSetting place = add(new BooleanSetting("Place", true, v -> page.getValue() == Page.Interact).setParent());
    private final SliderSetting placeDelay = add(new SliderSetting("PlaceDelay", 300, 0, 1000, v -> page.getValue() == Page.Interact && place.isOpen()).setSuffix("ms"));
    private final EnumSetting<SwapMode> autoSwap = add(new EnumSetting<>("AutoSwap", SwapMode.Off, v -> page.getValue() == Page.Interact && place.isOpen()));
    private final BooleanSetting afterBreak = add(new BooleanSetting("AfterBreak", true, v -> page.getValue() == Page.Interact && place.isOpen()));
    private final BooleanSetting breakSetting = add(new BooleanSetting("Break", true, v -> page.getValue() == Page.Interact).setParent());
    private final SliderSetting breakDelay = add(new SliderSetting("BreakDelay", 300, 0, 1000, v -> page.getValue() == Page.Interact && breakSetting.isOpen()).setSuffix("ms"));
    private final BooleanSetting breakRemove = add(new BooleanSetting("Remove", false, v -> page.getValue() == Page.Interact && breakSetting.isOpen()));
    //Render
    private final ColorSetting text = add(new ColorSetting("Text", new Color(-1), v -> page.getValue() == Page.Render).injectBoolean(true));
    private final BooleanSetting render = add(new BooleanSetting("Render", true, v -> page.getValue() == Page.Render));
    private final BooleanSetting sync = add(new BooleanSetting("Sync", true, v -> page.getValue() == Page.Render && render.getValue()));
    private final BooleanSetting shrink = add(new BooleanSetting("Shrink", true, v -> page.getValue() == Page.Render && render.getValue()));
    private final ColorSetting box = add(new ColorSetting("Box", new Color(255, 255, 255, 255), v -> page.getValue() == Page.Render && render.getValue()).injectBoolean(true));
    private final SliderSetting lineWidth = add(new SliderSetting("LineWidth", 1.5d, 0.01d, 3d, 0.01, v -> page.getValue() == Page.Render && render.getValue()));
    private final ColorSetting fill = add(new ColorSetting("Fill", new Color(255, 255, 255, 100), v -> page.getValue() == Page.Render && render.getValue()).injectBoolean(true));
    private final SliderSetting sliderSpeed = add(new SliderSetting("SliderSpeed", 0.2, 0.01, 1, 0.01, v -> page.getValue() == Page.Render && render.getValue()));
    private final SliderSetting startFadeTime = add(new SliderSetting("StartFade", 0.3d, 0d, 2d, 0.01, v -> page.getValue() == Page.Render && render.getValue()).setSuffix("s"));
    private final SliderSetting fadeSpeed = add(new SliderSetting("FadeSpeed", 0.2d, 0.01d, 1d, 0.01, v -> page.getValue() == Page.Render && render.getValue()));
    ...
    //Calc
    private final BooleanSetting useThread = add(new BooleanSetting("UseThread", true, v -> page.getValue() == Page.Calc));
    private final BooleanSetting doCrystal = add(new BooleanSetting("CalcDoCrystal", false, v -> page.getValue() == Page.Calc));
    private final BooleanSetting lite = add(new BooleanSetting("Lite", false, v -> page.getValue() == Page.Calc));
    ...
    private final SliderSetting predictTicks = add(new SliderSetting("Predict", 4, 0, 10, v -> page.getValue() == Page.Calc).setSuffix("ticks"));
    private final BooleanSetting terrainIgnore = add(new BooleanSetting("TerrainIgnore", true, v -> page.getValue() == Page.Calc));
    //Misc
    private final BooleanSetting constantMine = add(new BooleanSetting("ConstantMine", true, v -> page.getValue() == Page.Misc).setParent());
    private final SliderSetting constantProgress = add(new SliderSetting("Progress", 90.0, 0.0, 100.0, v -> page.getValue() == Page.Misc && constantMine.isOpen()).setSuffix("%"));
    private final BooleanSetting antiSurround = add(new BooleanSetting("AntiSurround", false, v -> page.getValue() == Page.Misc).setParent());
    private final SliderSetting antiSurroundMax = add(new SliderSetting("WhenLower", 5.0, 0.0, 36.0, v -> page.getValue() == Page.Misc && antiSurround.isOpen()).setSuffix("dmg"));
    private final BooleanSetting slowPlace = add(new BooleanSetting("Timeout", true, v -> page.getValue() == Page.Misc).setParent());
    private final SliderSetting slowDelay = add(new SliderSetting("TimeoutDelay", 600, 0, 2000, v -> page.getValue() == Page.Misc && slowPlace.isOpen()).setSuffix("ms"));
    private final SliderSetting slowMinDamage = add(new SliderSetting("TimeoutMin", 1.5, 0.0, 36.0, v -> page.getValue() == Page.Misc && slowPlace.isOpen()).setSuffix("dmg"));
    private final BooleanSetting forcePlace = add(new BooleanSetting("ForcePlace", true, v -> page.getValue() == Page.Misc).setParent());
    private final SliderSetting forceMaxHealth = add(new SliderSetting("LowerThan", 7, 0, 36, v -> page.getValue() == Page.Misc && forcePlace.isOpen()).setSuffix("health"));
    private final SliderSetting forceMin = add(new SliderSetting("ForceMin", 1.5, 0.0, 36.0, v -> page.getValue() == Page.Misc && forcePlace.isOpen()).setSuffix("dmg"));
    private final BooleanSetting armorBreaker = add(new BooleanSetting("ArmorBreaker", true, v -> page.getValue() == Page.Misc).setParent());
    private final SliderSetting maxDurable = add(new SliderSetting("MaxDurable", 8, 0, 100, v -> page.getValue() == Page.Misc && armorBreaker.isOpen()).setSuffix("%"));
    private final SliderSetting armorBreakerDamage = add(new SliderSetting("BreakerMin", 3.0, 0.0, 36.0, v -> page.getValue() == Page.Misc && armorBreaker.isOpen()).setSuffix("dmg"));
    private final SliderSetting hurtTime = add(new SliderSetting("HurtTime", 10, 0, 10, 1, v -> page.getValue() == Page.Misc));
    public PlayerEntity displayTarget;
    public float breakDamage, tempDamage, lastDamage, lastYaw, lastPitch;
    ...
    public AutoCrystal() {
        ...
        INSTANCE = this;
        FlawLess.EVENT_BUS.subscribe(new CrystalRender());
    }
    ...
    public String getInfo() {
        if (displayTarget != null && lastDamage > 0) {
            ...
            return lastBreakTimer.getPassedTimeMs() + "ms, " + new DecimalFormat("0.0").format(lastDamage);
        }
        return null;
    }
    ...
    public void onRotate(RotateEvent event) {
        if (rotate.getValue() && yawStep.getValue() && directionVec != null && !noPosTimer.passed(1000)) {
            ...
            if (random.getValue() && rd.nextBoolean() && rd.nextBoolean()) {
                lastPitch = Math.min(rd.nextFloat() * 2 + lastPitch, 90);
            }
    ...
    private void update() {
        ...
        if (list.isEmpty()) {
            ...
            if (tempPos != null) {
                debug("PlacePos Found");
            }
            debug("Start BreakForeach");
            ...
            if (breakPos != null) {
                debug("BreakPos Found");
            }
            if (doCrystal.getValue() && breakPos != null) {
                doBreak(breakPos);
                breakPos = null;
            }
            if (antiSurround.getValue() && PacketMine.breakPos != null && PacketMine.progress >= 0.9 && !BlockUtil.hasEntity(PacketMine.breakPos, false)) {
                if (tempDamage <= antiSurroundMax.getValueFloat()) {
                    for (PlayerAndPredict pap : list) {
                        for (Direction i : Direction.values()) {
                            ...
                            if (offsetPos.equals(PacketMine.breakPos)) {
    ...
    public boolean canPlaceCrystal(BlockPos pos, boolean ignoreCrystal, boolean ignoreItem) {
        ...
        return (getBlock(obsPos) == Blocks.BEDROCK || getBlock(obsPos) == Blocks.OBSIDIAN)
                ...
                && (BlockUtil.isAir(boost) || hasCrystal(boost) && getBlock(boost) == Blocks.FIRE)
                && (!CombatSetting.INSTANCE.lowVersion.getValue() || BlockUtil.isAir(boost2));
    }
    ...
    public float calculateDamage(Vec3d pos, PlayerEntity player, PlayerEntity predict) {
        if (constantMine.getValue() && PacketMine.breakPos != null) {
            if (mc.player.getEyePos().distanceTo(PacketMine.breakPos.toCenterPos()) <= PacketMine.INSTANCE.range.getValue()) {
                if (PacketMine.progress >= constantProgress.getValue() / 100) {
                    CombatUtil.modifyPos = PacketMine.breakPos;
        ...
        switch (calcType.getValue()) {
            ...
            case FlawLess -> damage = ExplosionUtil.calculateDamage(pos.getX(), pos.getY(), pos.getZ(), player, predict, 6);
        }
    ...
    private void debug(String s) {
        if (debug.getValue()) CommandManager.sendChatMessage(s);
    }

    private enum Page {
        General, Interact, Misc, Rotate, Calc, Render
    }
    ...
    private class CrystalRender {
        ...
        public void onRender3D(Render3DEvent event) {
            ...
            if (text.booleanValue && lastDamage > 0) {
                Render3DUtil.drawText3D(lastDamage + "", curVec3d, text.getValue());
            }
        }
    }
}
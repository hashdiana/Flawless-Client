...
import static me.flawless.api.utils.world.BlockUtil.getWorldActionId;

public class BedAura extends Module {
	public static BedAura INSTANCE;
	public final EnumSetting<Page> page = add(new EnumSetting<>("Page", Page.General));
	//General
	private final BooleanSetting legacy = add(new BooleanSetting("Legacy", false, v -> page.getValue() == Page.General));
	private final BooleanSetting spam = add(new BooleanSetting("Spam", true, v -> page.getValue() == Page.General));
	private final BooleanSetting onlyGround = add(new BooleanSetting("OnlyGround", true, v -> page.getValue() == Page.General));
	...
	private final EnumSetting<AutoAnchor.CalcMode> calcType = add(new EnumSetting<>("DMGCalcType", AutoAnchor.CalcMode.FlawLess, v -> page.getValue() == Page.General));
	...
	public void onRotate(RotateEvent event) {
		if (!rotate.getValue() && yawDeceive.getValue()) {
			event.setYaw(yaw);
		} else if (placePos != null && newRotate.getValue() && directionVec != null) {
	...
	private void doPlace(BlockPos pos) {
		...
		for (Direction i : Direction.values()) {
			...
			if ((legacy.getValue() && BlockUtil.canReplace(pos.offset(i)) || BlockUtil.clientCanPlace(pos.offset(i), false)) && BlockUtil.canClick(pos.offset(i).down()) && (!checkMine.getValue() || !BlockUtil.isMining(pos.offset(i)))) {
				facing = i;
				break;
			}
		}
		if (facing != null) {
			...
			yaw = HoleKick.getYaw(facing.getOpposite());
	...
	private boolean canPlaceBed(BlockPos pos) {
		if (BlockUtil.canReplace(pos) && (!checkMine.getValue() || !BlockUtil.isMining(pos)) && (!legacy.getValue() || BlockUtil.canClick(pos.down()))) {
			for (Direction i : Direction.values()) {
				if (i == Direction.UP || i == Direction.DOWN) continue;
	...
	private boolean isTrueFacingNow(Direction facing) {
		return Direction.fromRotation(FlawLess.ROTATE.lastYaw) == facing;
	}
	public enum Page {
		General,
		Rotate,

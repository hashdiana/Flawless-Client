...
public class Step extends Module {
	...
	public final BooleanSetting useTimer =
			add(new BooleanSetting("Timer", true, v -> mode.getValue() == Mode.OldNCP || mode.getValue() == Mode.NCP));
	public final BooleanSetting fast =
			add(new BooleanSetting("Fast", true, v -> mode.getValue() == Mode.NCP && useTimer.getValue()));
	...
	public void onUpdate() {
		if (nullCheck()) return;
		if (sneakingPause.getValue() && mc.player.isSneaking() || inBlockPause.getValue() && EntityUtil.isInsideBlock() || mc.player.isInLava() || mc.player.isTouchingWater() || inWebPause.getValue() && HoleKick.isInWeb(mc.player) || !mc.player.isOnGround() || onlyMoving.getValue() && !MovementUtil.isMoving()) {
			mc.player.setStepHeight(0.6f);
			return;
		}
		...
		if (mode.getValue().equals(Mode.OldNCP) || strict) {
			...
			if (offsets != null && offsets.length > 1) {
				if (useTimer.getValue()) {
					FlawLess.TIMER.set((float) getTimer(stepHeight));
					timer = true;
					packets = 2;
				}

...
import static me.flawless.api.utils.world.BlockUtil.canReplace;
...
public class Burrow extends Module {
    ...
    public void onUpdateWalking(UpdateWalkingPlayerEvent event) {
        ...
        int block;
        if ((block = getBlock()) == -1) {
            CommandManager.sendChatMessageWidthId("§e[?] §c§oObsidian" + (enderChest.getValue() ? "/EnderChest" : "") + "?", hashCode());
            disable();
            return;
        }
        progress = 0;
        ...
        if (this.rotate.getValue() == RotateMode.Bypass) {
            event.cancelRotate();
            EntityUtil.sendYawAndPitch(FlawLess.ROTATE.rotateYaw, 90);
        }
        ...
        switch (above ? aboveLagMode.getValue() : lagMode.getValue()) {
            ...
            case Invalid -> //for (int i = 0; i < 20; i++)
            ...
            case ToVoid -> mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(mc.player.getX(), CombatSetting.INSTANCE.lowVersion.getValue() ? -7 : -70, mc.player.getZ(), false));
            case Rotation -> {
    ...
    private void gotoPos(BlockPos offPos) {
        if (rotate.getValue() == RotateMode.None) {
            ...
            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(offPos.getX() + 0.5, mc.player.getY() + 0.2, offPos.getZ() + 0.5, FlawLess.ROTATE.rotateYaw, 90, false));
        }
    }

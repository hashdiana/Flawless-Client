...
public class AutoTrap
    ...
    private final EnumSetting<Mode> headMode = add(new EnumSetting<>("HeadMode", Mode.Anchor));
    ...
    public AutoTrap() {
        ...
        INSTANCE = this;
        FlawLess.EVENT_BUS.subscribe(new AutoTrapRender());
    }
    ...
    private void doTrap(BlockPos pos) {
        ...
        if (legs.getValue()) {
            for (Direction i : Direction.values()) {
                ...
                tryPlaceBlock(offsetPos, legAnchor.getValue(), false);
        ...
        if (headExtend.getValue()) {
            for (int x : new int[]{1, 0, -1}) {
                for (int z : new int[]{1, 0, -1}) {
                    ...
                    if (checkEntity(new BlockPos(offsetPos))) tryPlaceBlock(offsetPos.up(2), headMode.getValue() == Mode.Anchor, headMode.getValue() == Mode.Concrete);
                }
            }
        }
        if (head.getValue()) {
            if (BlockUtil.clientCanPlace(pos.up(2), breakCrystal.getValue())) {
                ...
                tryPlaceBlock(pos.up(2), headMode.getValue() == Mode.Anchor, headMode.getValue() == Mode.Concrete);
            }
        }
    ...
    private void tryPlaceBlock(BlockPos pos, boolean anchor, boolean sand) {
        ...
        int block = sand ? getConcrete() : (anchor && getAnchor() != -1 ? getAnchor() : getBlock());
        if (block == -1) return;
        if (!pre.getValue()) AutoTrapRender.addBlock(pos);
        placeList.add(pos);

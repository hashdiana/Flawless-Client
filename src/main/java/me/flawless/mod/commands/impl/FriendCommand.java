...
public class FriendCommand extends Command {
	...
	public void runCommand(String[] parameters) {
		...
		if (parameters[0].equals("add")) {
			if (parameters.length == 2) {
				FlawLess.FRIEND.addFriend(parameters[1]);
				CommandManager.sendChatMessage("§a[√] §b" + parameters[1] + (FlawLess.FRIEND.isFriend(parameters[1]) ? " §ahas been friended" : " §chas been unfriended"));
				return;
			}
			sendUsage();
			return;
			...
			if (parameters.length == 2) {
				FlawLess.FRIEND.removeFriend(parameters[1]);
				CommandManager.sendChatMessage("§a[√] §b" + parameters[1] + (FlawLess.FRIEND.isFriend(parameters[1]) ? " §ahas been friended" : " §chas been unfriended"));
				return;
			}
			sendUsage();
		...
		if (parameters.length == 1) {
			CommandManager.sendChatMessage("§a[√] §b" + parameters[0] + (FlawLess.FRIEND.isFriend(parameters[0]) ? " §ais friended" : " §cisn't friended"));
			return;
		}
	...
	public String[] getAutocorrect(int count, List<String> seperated) {
		if (count == 1) {
			...
			for (String x : list) {
				if (input.equalsIgnoreCase(FlawLess.PREFIX + "friend") || x.toLowerCase().startsWith(input)) {
					correct.add(x);
				}
			}
